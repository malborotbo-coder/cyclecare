...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 backdrop-blur-lg bg-gradient-to-b from-black/60 via-black/40 to-black/30 border-b border-white/10">
        <div className="container mx-auto px-4 py-2 flex items-center justify-between gap-2">
          <div className="flex-shrink-0 hover-elevate active-elevate-2 transition-all duration-300 rounded-lg p-1">
            <Logo size="sm" />
          </div>
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  ...
                  title={lang === 'ar' ? 'البيانات الشخصية' : 'Profile'}
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel className="text-sm font-semibold">
                  {isNative 
                    ? (nativeUser?.firstName || nativeUser?.email || 'User')
                ...
                {(isNative ? nativeUser?.isAdmin : (user as any)?.isAdmin) && (
                ...
                {isNative && nativeUser?.email === 'user@cyclecatrtec.com' && (
                  <>
                    <DropdownMenuItem onClick={handleSwitchToRealAccount} data-testid="menu-item-use-real-account">
                      <span>{lang === 'ar' ? 'استخدم حسابك الحقيقي' : 'Use Real Account'}</span>
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                  </>
                )}
